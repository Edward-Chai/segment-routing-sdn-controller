...
class Te_controller(ControllerBase):
    ...
    def _process_hello(self, src_router_id, desig_router_id, nbors):
        LOG.info(
            "**HELLO**: src_router_id:%s, designated_router:%s, nbors:%s" % (src_router_id, desig_router_id, nbors))
        graph = Te_controller.graph
        g = graph.getG()
    ...
    def _process_rtr_lsa(self, lsa, lsid, advrtr):
        ...
        for intf in lsa['INTERFACES']:
            ...
            if src_router_adj:  # update
                ...
                src_router_adj = IntraAdj(LSID=src_intf_id, W=w, Prefixes=None, SrcRouterID=src_router_id,
                                          DstRouterID=dst_router_id, DstRouter=dst_router, SrcInterfaceID=src_intf_id,
                                          SrcInterfaceAddr=None, DstInterfaceID=dst_intf_id)
            ...
            if dst_router_adj:  # update
                ...
                dst_router_adj = IntraAdj(LSID=dst_intf_id, W=w, Prefixes=None, SrcRouterID=dst_router_id,
                                          DstRouterID=advrtr, DstRouter=src_router, SrcInterfaceID=dst_intf_id,
                                          SrcInterfaceAddr=None, DstInterfaceID=src_intf_id)
    ...
    def _process_link_lsa(self, lsa, lsid, advrtr):
        ...
        if src_router_adj:
            ...
            src_router_adj = IntraAdj(LSID=lsid, W=0, Prefixes=prefixes, SrcRouterID=src_router_id, DstRouterID=None,
                                      DstRouter=None, SrcInterfaceID=lsid, SrcInterfaceAddr=lladdr, DstInterfaceID=None)
            src_router.IntraAdjs.append(src_router_adj)
    ...
    def _process_network_lsa(self, lsa, lsid, advrtr):
        ...
        for rtr in lsa['RTRS']:
            ...
            if src_router_adj:  # update adj
                ...
                src_router_adj = IntraAdj(LSID=lsid, W=0, Prefixes=None, SrcRouterID=src_router_id,
                                          DstRouterID=dst_router_id, DstRouter=dst_router, SrcInterfaceID=None,
                                          SrcInterfaceAddr=None, DstInterfaceID=None)
                src_router.IntraAdjs.append(src_router_adj)
            ...
            if dst_router_adj:  # update adj
                ...
                dst_router_adj = IntraAdj(LSID=None, W=0, Prefixes=None, SrcRouterID=dst_router_id,
                                          DstRouterID=src_router_id, DstRouter=src_router, SrcInterfaceID=None,
                                          SrcInterfaceAddr=None, DstInterfaceID=None)
                dst_router.IntraAdjs.append(dst_router_adj)
    ...
    def _process_intraareaprefix_lsa(self, lsa, lsid, advrtr):
        ...
        prefixes = lsa['prefixes']
        LOG.info("**Intra Area Prefix LSA**: lsid:%s, advrtr:%s, reflsid:%s, refadvrouterid:%s" % (
            lsid, advrtr, reflsid, src_router_id))
        ...
        if src_router_adj:
            ...
            src_router_adj = IntraAdj(LSID=reflsid, W=0, Prefixes=prefixes, SrcRouterID=src_router_id, DstRouterID=None,
                                      DstRouter=None, SrcInterfaceID=reflsid, SrcInterfaceAddr=None,
                                      DstInterfaceID=None)
            src_router.IntraAdjs.append(src_router_adj)
    ...
    def _remove_adjs(self, router_id, router_adjs):
        ...
        for adj in g[router_id].IntraAdjs:
            LOG.debug(
                "Removing adj: router:%s, dst_router:%s, router_adjs:%s" % (router_id, adj.DstRouterID, router_adjs))
            if adj.DstRouterID != None and not adj.DstRouterID in router_adjs[router_id]:
                ...
                if adj.DstRouterID != None:
                    ...
                    LOG.debug("Removed adj: router:%s, dst_router:%s, router_adjs:%s" % (
                        adj.DstRouterID, router_id, router_adjs))
                LOG.debug(
                    "Removed adj: router:%s, dst_router:%s, router_adjs:%s" % (router_id, adj.DstRouterID, router_adjs))
        return 0
    ...
    def _update_adj(self, src_router_id, dst_router_id, src_intf_id=None, dst_intf_id=None, src_router_prefixes=None,
                    src_router_lladdr=None, W=None):
        LOG.debug(
            "update adj: src_router_id:%s, dst_router_id:%s, src_intf_id:%s, dst_intf_id:%s, src_router_prefixes:%s, src_router_lladdr:%s, w:%s" % (
                src_router_id, dst_router_id, src_intf_id, dst_intf_id, src_router_prefixes, src_router_lladdr, W))
        g = Te_controller.graph.getG()
        r1 = 0
        r2 = 0
        if src_router_id != None and dst_router_id != None:
            new_adj = self._fetch_adj_by_router_ids(src_router_id, dst_router_id, g)
            r1 = self._update_adj_by_router_ids(src_router_id, dst_router_id, new_adj, src_intf_id, dst_intf_id,
                                                src_router_prefixes, src_router_lladdr, W, g)
        if src_router_id != None and src_intf_id != None:
            new_adj = self._fetch_adj_by_lsid(src_router_id, src_intf_id, g)
            r2 = self._update_adj_by_lsid(src_router_id, new_adj, dst_router_id, src_intf_id, dst_intf_id,
                                          src_router_prefixes, src_router_lladdr, W, g)
        return max(r1, r2)

    def _update_adj_by_lsid(self, src_router_id, new_adj, dst_router_id=None, src_intf_id=None, dst_intf_id=None,
                            src_router_prefixes=None, src_router_lladdr=None, W=None, g=None):
        if not g:
        ...
        for i in range(0, len(g[src_router_id].IntraAdjs)):
            if lsid == g[src_router_id].IntraAdjs[i].LSID:
                new_adj._update(src_router_id, dst_router_id, src_intf_id, dst_intf_id, src_router_prefixes,
                                src_router_lladdr, W)
                g[src_router_id].IntraAdjs[i] = new_adj
                LOG.debug("Updated: _update_adj_by_lsid")
        return 0

    def _update_adj_by_router_ids(self, src_router_id, dst_router_id, new_adj, src_intf_id=None, dst_intf_id=None,
                                  src_router_prefixes=None, src_router_lladdr=None, W=None, g=None):
        if not g:
            LOG.warn("The topology graph is empty!")
            return -1
