...
class IntraAdj(object):
    ...
    def _update(self, src_router_id=None, dst_router_id=None, src_intf_id=None, dst_intf_id=None,
                src_router_prefixes=None, src_router_lladdr=None, W=None):
        if src_intf_id != None:
    ...
    def print_me(self):
        LOG.info("LS:\n"
                 ...
                 "\t	W:%s\n"
                 "\t	Prefixes:%s\n" % (
                 self.D, self.LSID, self.SrcRouterID, self.DstRouterID, self.DstRouter, self.SrcInterfaceID,
                 self.DstInterfaceID, self.SrcInterfaceAddr, self.W, self.Prefixes)
                 )
...
class G(object):
    ...
    def addAdj(self, src_router_id, dst_router_id):
        ...
        # src->dst
        src_router_adj = IntraAdj(LSID=None, W=None, Prefixes=None, SrcRouterID=src_router_id,
        ...
        # dst->src
        dst_router_adj = IntraAdj(LSID=None, W=None, Prefixes=None, SrcRouterID=dst_router_id,
    ...
    def print_me(self):
        for ID in self.G:
            ...
            for adj in self.G[ID].IntraAdjs:
                if adj.D == 0:
                    LOG.info(
                        "\tsrcRtrID:%s,LSID:%s,W:%s,Prefixes:%s,srcIntfID:%s,srcIntfAddr:%s,dstIntfID:%s,dstRtrId:%s\n" % (
                        adj.SrcRouterID, adj.LSID, adj.W, adj.Prefixes, adj.SrcInterfaceID, adj.SrcInterfaceAddr,
                        adj.DstInterfaceID, adj.DstRouterID))
